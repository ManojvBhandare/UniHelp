...
export const NavBar = ({ scrollToSection, refs }) => {
  ...
  return (
    <div className="flex items-center justify-between w-[80%] h-[4rem] md:h-[5rem] mt-[1rem] md:mt-[1rem] p-[1rem] bg-black text-white rounded-xl shadow-md z-50 fixed top-0 left-[10%]">
      <button className="flex" onClick={() => scrollToSection(refs.homeRef)}>
      ...
      >
        <button
          className=" md:text-[1rem] text-[0.875rem] my-2 md:my-0"
          onClick={() => scrollToSection(refs.studentsRef)}
        >
          For Students
        </button>
        <button
          className="md:text-[1rem] text-[0.875rem] my-2 md:my-0"
          onClick={() => scrollToSection(refs.teachersRef)}
        >
          For Teachers
        </button>
        <button
